package com.ducks.goodsduck.commons.repository.pricepropose;
...
public class PriceProposeRepositoryCustomImpl implements PriceProposeRepositoryCustom {
    ...
    @Override
    public List<PricePropose> findAllByUserIdWithAllStatus(Long userId) {
        return queryFactory
                .select(pricePropose)
                .from(pricePropose)
                .where(pricePropose.user.id.eq(userId))
                .orderBy(pricePropose.id.desc())
                .fetch();
    }
